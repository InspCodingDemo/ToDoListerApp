using CodeToolbox.Framework;
using ToDoListerApp.Core.Features;
using ToDoListerApp.Core.Features.AddToDoTask;
using ToDoListerApp.Core.Framework;
using static CodeToolbox.Logic.StringHelper;

namespace ToDoListerApp.Core.Logic
{
    public class ToDoManager(ILogger<ToDoManager> logger, LocalStorageContext localStorageContext)
    {
        ...
        {
            if(GetTotalCount() == 0 && localStorageContext is not null)
            {
